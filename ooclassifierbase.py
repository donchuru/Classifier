...
class TrainingInstance(C274):
    ...
    def preprocess_words(mode = ''):
        import sys

        #if (len(sys.argv) == 1) or (sys.argv[1] == 'keep-digits') or (sys.argv[1] == 'keep-stops') or (sys.argv[1] == 'keep-symbols'):
        #GET RID OF THE INPUT() words = input().lower()
            words = list(words.split())
        else:
            print('''Error: Invalid input.\nUsage: python3 preprocess.py <mode>.\nMode can be one of "keep-digits", "keep-stops" or "keep-symbols".''')
            sys.exit()
        ...
        #initializing new blank list where i shall store the preprocessed words
        new_words = []

        def symb_punct_removal(words):
            for i in words:
                if i.isalnum() == True:
                    new_words.append(i)
                else:
        ...
        def num_removal(a):
            ...
            for i in a:
                if i.isdigit() == True: 
                    new_words2.append(i)
                elif any(char.isdigit() for char in i) == False:
                    new_words2.append(i)
                elif any(char.isdigit() for char in i) == True:
                    #seperate each element and check if each of them is a digit and if so
                    #replace the digit with nothing(removing it)
                    new_i = ''.join('' if char.isdigit() else char for char in i)
                    new_words2.append(new_i)
                else:
                    new_words2.append(i)
            return new_words2
                        
        def stopword_removal(b):
            for i in list(b):
                if i in stopwords:
                    b.remove(i)
                if i in list(b) == ' ':
        ...
        def edited_text():
            ...
            if mode == '':
                new_words1 = symb_punct_removal(words)
                new_words2 = num_removal(new_words1)
                new_words3 = stopword_removal(new_words2)
                processedwords = ' '.join(new_words3)
                processedwords = processedwords.strip()
                return(processedwords)
            elif mode == 'keep-digits':
                new_words2 = symb_punct_removal(words)
                new_words3 = stopword_removal(new_words2)
                processedwords = ' '.join(new_words3)
                processedwords.strip()
                return(processedwords)
            elif mode == 'keep-stops':
                new_words1 = symb_punct_removal(words)
                new_words2 = num_removal(new_words1)
                processedwords = ' '.join(new_words2)
                processedwords.strip()
                return(processedwords)
            elif mode == 'keep-symbols':
                new_words2 = num_removal(words)
                new_words3 = stopword_removal(new_words2)
                processedwords = ' '.join(new_words3)
                processedwords.strip()
                return(processedwords)
...
class TrainingSet(C274):
    ...
    def preprocess(mode = ''):
        TrainingInstance.preprocess_words(mode = '')

##where i have reached(TASK 3)
    def return_nfolds(num=3):
        #returns list of num objects of class TrainingSet
        obj1 = TrainingSet()
    ...
    def add_training_set(tset):
        #adds all training instances of tset to an object of class TrainingSet


def ClassifyByTopN(ClassifyByTarget):
    def target_top_n(test,num = 5, label = ''):
        #this method replaces the current list of target words with a new list of target words based on word frequency.
